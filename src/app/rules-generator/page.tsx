...
function generateRulesContent(config: RulesConfig): string {
  ...
  content += `## ${lang.commitStyle}\n\n`;
  content += `${config.language === 'de' ? 'Verwende Conventional Commits:' : 'Use Conventional Commits:'}\n\n`;
  
  content += '```\n';
  content += `${config.useEmoji ? emojiMap.feat + ' ' : ''}feat: ${config.language === 'de' ? 'Neues Feature' : 'New feature'}\n`;
  content += `${config.useEmoji ? emojiMap.fix + ' ' : ''}fix: ${config.language === 'de' ? 'Bugfix' : 'Bug fix'}\n`;
  content += `${config.useEmoji ? emojiMap.docs + ' ' : ''}docs: ${config.language === 'de' ? 'Dokumentation' : 'Documentation'}\n`;
  content += `${config.useEmoji ? emojiMap.style + ' ' : ''}style: ${config.language === 'de' ? 'Formatierung' : 'Formatting'}\n`;
  content += `${config.useEmoji ? emojiMap.refactor + ' ' : ''}refactor: ${config.language === 'de' ? 'Code-Refactoring' : 'Code refactoring'}\n`;
  content += `${config.useEmoji ? emojiMap.test + ' ' : ''}test: ${config.language === 'de' ? 'Tests' : 'Tests'}\n`;
  content += `${config.useEmoji ? emojiMap.chore + ' ' : ''}chore: ${config.language === 'de' ? 'Wartung' : 'Maintenance'}\n`;
  content += '```\n\n';

  content += `## ${lang.codeStyle}\n\n`;
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            >
              ...
              <div className="rounded-2xl border border-border/50 bg-card/50 p-5">
                ...
                <div className="flex gap-2">
                  {[ { value: 'functional', label: 'Funktional' }, { value: 'oop', label: 'OOP' }, { value: 'mixed', label: 'Gemischt' }, ].map((style) => (
                    <button
                      key={style.value}
