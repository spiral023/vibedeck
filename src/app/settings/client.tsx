...
export function SettingsClient({ knowledgeExport, knowledgeCount }: SettingsClientProps) {
  ...
  const buildUiLibrariesMarkdown = () => {
    return [
      '# UI Bibliotheken',
      ...uiLibraries.map((lib) => [
        `## ${lib.name}`,
        `Kategorie: ${lib.category}`,
        `Beschreibung: ${lib.description}`,
        `URL: ${lib.url}`,
      ].join('\n')),
    ].join('\n\n');
  };

  const buildToolDirectoryMarkdown = () => {
    return [
      '# Tool Directory',
      ...toolCategories.map((category) => {
        const tools = category.tools
          .map((tool) => `- ${tool.name}: ${tool.description} (URL: ${tool.url})`)
          .join('\n');
  ...
  const buildGithubReposMarkdown = () => {
    return [
      '# GitHub Repos',
      ...githubRepos.map((repo) => [
        `## ${repo.name}`,
        `Kategorie: ${repo.category}`,
        `Autor: ${repo.author}`,
  ...
  const handleCopyStaticContent = async (label: string, content: string, count?: number) => {
    const success = await copyToClipboard(content);
    if (success) {
      toast.success(`${label} kopiert${typeof count === 'number' ? ` (${count})` : ''}`);
    } else {
    ...
    <div className="space-y-8 max-w-3xl">
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-border/50 bg-background/60 p-4 space-y-3">
            <div>
              <h3 className="font-semibold">UI Bibliotheken</h3>
              <p className="text-sm text-muted-foreground">{uiLibraryCount} Einträge.</p>
            </div>
            <button
              onClick={() => handleCopyStaticContent('UI Bibliotheken', buildUiLibrariesMarkdown(), uiLibraryCount)}
          ...
          <div className="rounded-xl border border-border/50 bg-background/60 p-4 space-y-3">
            ...
            <button
              onClick={() => handleCopyStaticContent('Tool Directory', buildToolDirectoryMarkdown(), toolCount)}
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-secondary px-4 py-2 text-sm font-medium text-secondary-foreground hover:bg-secondary/80 focus-ring"
            >
              <Copy className="h-4 w-4" />
              Tool Directory kopieren
            </button>
          </div>
          ...
          <div className="rounded-xl border border-border/50 bg-background/60 p-4 space-y-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{repoCount} Repos.</p>
            </div>
            <button
              onClick={() => handleCopyStaticContent('GitHub Repos', buildGithubReposMarkdown(), repoCount)}
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-secondary px-4 py-2 text-sm font-medium text-secondary-foreground hover:bg-secondary/80 focus-ring"
            >
              <Copy className="h-4 w-4" />
              GitHub Repos kopieren
            </button>
          </div>
          ...
          <div className="rounded-xl border border-border/50 bg-background/60 p-4 space-y-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{setupCount} Guides.</p>
            </div>
            <button
              onClick={() => handleCopyStaticContent('Setup', buildSetupMarkdown(), setupCount)}
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-secondary px-4 py-2 text-sm font-medium text-secondary-foreground hover:bg-secondary/80 focus-ring"
            >
              <Copy className="h-4 w-4" />
              Setup kopieren
